class SourceManagerController < ApplicationController
  ...
  def edit_node
    ...
    if  @all_node_detail.node_type == "Meter"
	  ...
	  #render :text => @meter_detail.inspect and return false
    end
  ...
  def update_node
	...
	if @node_type == "Meter"
	  NodeEntry.update_all({:name => params["meter_name"]},['id = ?', @node_id])
	  Meter.update_all({:meter_identifier => params["meter_meter_identifier"],:source_type_id => params["meter"]["source_type"],:meter_ip => params["meter_meter_ip"]},['id = ?', @node_node_id])
